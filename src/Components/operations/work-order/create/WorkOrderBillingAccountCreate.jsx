...
import { createWorkOrderCustomerAccount, deleteWorkOrderCustomerAccount, updateWorkOrderCustomerAccount, updateWorkOrderDefaultCustomerRecipient } from "../../../../Controller/api/workOrderServices";
...
class WorkOrderBillingAccountCreate extends Component {
  ...
  handleOnSelect = (obj) => {
    ...
    createWorkOrderCustomerAccount(values).then(() => {
  ...
  handleDeleteContactAccount = () => {
    deleteWorkOrderCustomerAccount(this.state.deletableAccountId).then(() => {
      this.props.fetchWorkOrder(this.props.workOrder.id);
    });
  };
  ...
  handleContactSelect = (id) => {
    // this.contactRef.current.setFieldsValue({
      ...
      const customer_contact_accounts = workOrder.work_customer_contact;
      ...
      updateWorkOrderCustomerAccount(contactModalItem.id, values).then(() => {
        this.props.fetchWorkOrder(this.props.workOrder.id);
      });
    };
  ...
  handleDeleteContact = () => {
    ...
    updateWorkOrderCustomerAccount(this.state.selectedItem.id, values).then(
      (response) => {
        this.props.fetchWorkOrder(this.props.workOrder.id);
      }
    );
  ...
  handleSubmit = () => {
    ...
    this.props.setWorkOrder(this.props.workOrder, 5);
  };
  ...
  handleDefaultRecipient = (itemId, contactId) => {
    ...
    updateWorkOrderDefaultCustomerRecipient(itemId, { default_contact_id: contactId })
      .catch((err) => {
        handleError(err);
      });
  };
  ...
  callbackContact = (data) => {
     ...
     const { contactModalItem } = this.state;
      ...
      updateWorkOrderCustomerAccount(contactModalItem.id, values).then((res) => {
        this.props.fetchWorkOrder(this.props.workOrder.id);
  ...
  render() {
    ...
    const selectedContactAccounts = this.props.workOrder?.work_customer_contact || [];
    return (
      <React.Fragment>
...
export default withRouter(WorkOrderBillingAccountCreate);
