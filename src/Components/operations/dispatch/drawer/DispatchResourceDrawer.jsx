...
class DispatchResourceDrawer extends Component {
  state = {
    crewChief: "",
    employees: [],
    search: "",
  ...
  handleSelect = (params) => {
      updateDispatchData(params, this.props.workOrder?.id).then((res) => {
  ...
  handleDispatchDataById = () => {
    getDispatchDataById(this.props?.workOrder?.id).then((res) => {
  ...
  handleCardStatus = (value,id) => {
    ...
    postDispatchData(params,id).then((res) => {
      this.props.handleDispatchNow(true)
      this.handleDispatchDataById()
          ...
          <div className="row mx-0 inner-modal-main-row">
            <div className="col-12">
              <div className="row mx-0 dispatch-word-card position-relative pb-0">
                <div className={"status-btn"}>{dispatchData?.status}</div>
                <div className="col-12 position-relative dispatch-card-heading">
                  <h6>
                    <div className="work-id">{dispatchData?.workorder?.id}</div>
                    <div className="project-name">
                      {dispatchData?.workorder?.project?.name}
                    </div>
                  </h6>
                </div>
                <div className="col-12">
                  <ul className="list-inline  ">
                    <li className="pl-0 text-left">
                      <img
                        ...
                        className="img-fluid"
                      />
                      {dispatchData?.workorder?.service_date
                        ? moment(dispatchData?.workorder?.service_date).format(
                            "MMM D ,YYYY"
                          )
                        : "-"}{" "}
                      |{" "}
                      {dispatchData?.workorder?.start_time
                        ? moment(
                            dispatchData?.workorder?.start_time,
                            "HH:mm"
                          ).format("HH:mm A")
                        : "-"}{" "}
                      -{" "}
                      {dispatchData?.workorder?.end_time
                        ? moment(
                            dispatchData?.workorder?.end_time,
                            "HH:mm"
                          ).format("HH:mm A")
                ...
                <div className="col-12 ">
                  {dispatchData.status === "IN_QUEUE" || "NOT_ACCEPTED" ? 
                  <Button
                    disabled={this.props.assignPending || !this.state.dispatchNow}
                    className="common-project-btn assign-btn dispatch-btn-new"
                    onClick={() => {
                      this.handleCardStatus("PENDING",dispatchData.id)
                    } 
                    }
                  >
                    dispatch now
                ...
                <div className="col-12 table-responsive main-table-div position-relative wage-table">
                  <div className="row mx-0 custom-table-main-row custom-table-main-row-proposal-line-item custom-table-main-row-wage-info-main proposal-update-table dispatch-service-varient">
                    ...
                    <div className="row mx-0 w-100">
                      {workOrder?.workorder?.workorder_variant.map((n) => {
                        return (
...
export default DispatchResourceDrawer;
