...
import ProjectsServiceVarientTableCustomView from "./ProjectsServiceVarientTableCustomView";
import { getServiceVariantProject } from '../../../../Controller/api/projectServices';
...
class ProjectsServiceVarientsView extends Component {
    constructor(props) {
        super(props);
        this.state = {
            allOptions: [],
    ...
    getSelectedServiceVariants = () => {
        ...
        getServiceVariantProject({ project: project?.id }).then(resp => {
        ...
        newPricing.forEach((item) => {
            if (item?.manually_added) {
                ...
                let secondCheckedValue =
                    (parseInt(item.project_qty) || 1) * parseFloat(pricing_value || 0);
                if (preTax) {
                    costPricing =
    ...
    getResourcePricingCost = () => {
        let newPricing = this.props.project.line_item;
        let cost = 0;
        newPricing.forEach((item) => {
            const qty = parseInt(item.project_qty) || 1;
            if (item.table_data) {
                let totalLaborQty = 0;
    ...
    render() {
        const { viewAll, project } = this.props;
            ...
            <div className={`col-12 ${!this.props.viewAll ? "mt-30" : ""}`}>
                ...
                {newPricing.length > 0 ?
                    <div
                        ...
                        <div
                            className="row mx-0 custom-table-main-row custom-table-main-row-proposal-line-item custom-table-main-row-wage-info-main proposal-update-table">
                            <div className="col-12 custom-table-change service-variants-table">
                                ...
                                <div className="row">
                                    {/* <ProjectsServiceVarientTableCustomView /> */}
                                    {newPricing?.map((n) => {
                    ...
                    <div className="col-12 p-0">
                        <div className="row line-items-row-main">
                            <div className="col-12 col-sm-4">
                                <div className="row mb-3">
                                    ...
                                    <div className="col-12">
                                        <span className="sub-total-text gray-1">
                                            ...
                                            {costSettingOptions.find((i) => project?.cost_setting === i.value)?.name || "Standard" }
                                        </span>
                                    </div>
                                </div>
                                <div className="row mb-3">
                                ...
                                <div className="row mb-3">
                                    ...
                                    <div className="col-12">
                                        <span className="sub-total-text gray-1">
                                            ...
                                            {paymentOptions.find((n) => n.value === project?.payment_terms)?.name || "30 Days" }
                                        </span>
                                    </div>
                                </div>
                                <div className="row mb-3">
                                    ...
                                    <div className="col-12">
                                        <span className="sub-total-text gray-1">
                                            {project?.deposit === "YES" ? "Yes" : project?.deposit === "NO" ? "No" : "-"}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="row">
                                    <div className="col-12">
                                        <span className="sub-total-text">Deposit Amount</span>
                                    </div>
                                    <div className="col-12">
                                        <span className="sub-total-text gray-1">
                                            {`$${project?.deposit_amount}`}
                            ...
                            <div className="col-12 col-sm-7 offset-sm-1">
                                    ...
                                    <div className="col-4 text-md-right">
                                        <span className="sub-total-text gray-1"> */}
                                            {/* ${this.getResourcePricingCost()} */}
                                             ...
                                             {/* ${formatPrice(project?.estimated_cost) || 0.00}
                                        </span>
                                    </div>
                                </div> */}
                                {/* <div className="row estimated-total-row">
                                    ...
                                    <div className="col-4 text-md-right">
                                        <span className="sub-total-text gray-1"> */}
                                            {/* {this.calculatedProfitMargin()}% */}
                                             {/* {project?.profit_margin == 0 ? "-" : project?.profit_margin} %
                                        </span>
                                    </div>
                                </div> */}
                                <div className="row estimated-total-row">
                                    ...
                                    <div className="col-4 text-md-right">
                                        <span className="sub-total-text gray-1">
                                            {(project?.cost_setting == "LUMP_SUM")
                                        ||
                                            (project?.cost_setting == "LUMP_SUM_WITH_UOM_AND_QTY")
                                            ?
                                            <>${formatPrice(project?.estimated_total_price_pre_tax)}</>
                                            :
                                    ...
                                    <div className="col-5 text-md-right">
                                        <ul className="list-inline mb-0">
                                            <li className="list-inline-item">
                                                <small>
                                                    {project?.tax_basis?.percentage != 0 ? project?.tax_basis?.percentage : "-"}
                                                    </small>
                                            </li>
                                            ...
                                            <li className="list-inline-item">
                                                <span className="sub-total-text gray-1">
                                                    {/* ${this.calculatedEstimatedTaxes()} */}
                                                     ${formatPrice(project?.estimated_taxes) || 0.00}
                                                </span>
                                            </li>
                                        </ul>
                                    </div>
                                ...
                                <div className={`${project?.cost_setting == "TOTAL_PRICE_PER_UNIT" && "normal-text"} row estimated-total-row-3 estimated-total-row`}>
                                {/* <div className={`row estimated-total-row-3 estimated-total-row`}> */}
                                    ...
                                    <div className="col-5 text-md-right">
                                        {(project?.cost_setting == "LUMP_SUM" || project?.cost_setting == "LUMP_SUM_WITH_UOM_AND_QTY") &&
                                        ...
                                        <span className="sub-total-text-main">
                                            {/* ${this.calculatedEstimatedTotal()} */}
                                             ...
                                             ${formatPrice(project?.estimated_total)?.toLocaleString()}
                                        </span>
                                    </div>
                                </div>
                                <div className="row estimated-total-row-2 pt-1 pb-0 estimated-total-row">
                                    ...
                                    <div className="col-5 text-md-right">
                                        <ul className="list-inline mb-0">
                                            ...
                                            <li className="list-inline-item">
                                                <span className="sub-total-text gray-1">
                                                    ${formatPrice(project?.estimated_profit) || 0.00}
                                                </span>
                                            </li>
                                        </ul>
                                    </div> */}
                                ...
                                {/*    </div>*/}
                                {/*</div>*/}
                                {project?.cost_setting === "TOTAL_PRICE_PER_UNIT" && (
                        <>
                          <div className="row estimated-total-row-2 pt-1 pb-0 estimated-total-row align-items-center">
                            ...
                            <div className="col-7 text-md-right value-div-inner">
                              <ul className="list-inline mb-0">
                                <li className="list-inline-item">
                                  <Input disabled placeholder={project?.project_uom ? `${project?.project_uom?.name} (${project?.project_uom?.symbol})` : "Select"} />
                                </li>
                                <li className="list-inline-item">
                                  <InputNumber
                                    disabled
                                    value={project?.total_units || 0}
                                  />
                                </li>
                              </ul>
                            </div>
                ...
                >
                    <div className="col-12 col-sm-4">
                        <div className="row">
                            ...
                            {project?.qualifiers?.split('^')?.map(item => {
                                return (
                                <div className="col-12">
                    ...
                    <div className="col-12 col-sm-7 offset-sm-1">
                        <div className="row">
                            ...
                            {project?.comments?.split('^')?.map(item => {
                            return (
                            <div className="col-12">
                                <span className="sub-total-text gray-1">{item != "<br>"? ('•'+" "+item).replace(/\&nbsp;/g, ''):""}</span>
                            </div>
...
export default ProjectsServiceVarientsView;