...
import { createProjectCustomerAccount, deleteProjectCustomerAccount, getProjectCustomerAccount, updateProjectCustomerAccount, updateProjectDefaultCustomerRecipient } from "../../../../Controller/api/projectServices";
...
class ProjectsBillingAccountCreate extends Component {
  ...
  handleOnSelect = (obj) => {
    ...
    createProjectCustomerAccount(values).then(() => {
  ...
  handleDeleteContactAccount = () => {
    deleteProjectCustomerAccount(this.state.deletableAccountId).then(() => {
      this.props.fetchProject(this.props.project.id);
    });
  };
  ...
  handleContactSelect = (id) => {
    ...
    const customer_contact_accounts = project.project_customer_contact;
    ...
    updateProjectCustomerAccount(contactModalItem.id, values).then(() => {
      this.props.fetchProject(this.props.project.id);
    });
  };
  ...
  handleDeleteContact = () => {
    ...
    updateProjectCustomerAccount(this.state.selectedItem.id, values).then(
      (response) => {
        this.props.fetchProject(this.props.project.id);
      }
    );
  ...
  handleSubmit = () => {
    ...
    this.props.setProject(this.props.project, 4);
  };
  ...
  handleDefaultRecipient = (itemId, contactId) => {
    ...
    updateProjectDefaultCustomerRecipient(itemId, { default_contact_id: contactId })
      .then()
      .catch((err) => {
        handleError(err);
      });
  ...
  callbackContact = (data) => {
    ...
    updateProjectCustomerAccount(contactModalItem.id, values).then((res) => {
      this.props.fetchProject(this.props.project.id);
  ...
  render() {
    ...
    const selectedContactAccounts = this.props.project?.project_customer_contact || [];
    return (
      <React.Fragment>
        <div className="row common-form-card-row">
          <div className="col-12">
            <div className="row mx-0 info-gray-div align-items-center">
              <h6 className="mb-0">
                You can choose one billing account per project.
              </h6>
            </div>
          </div>
                ...
                <div className="col-12 p-0">
                  ...
                  >
                    <Select
                      ...
                      // disabled={!this.props.location.pathname?.includes('create') && selectedContactAccounts.length === 1}
...
export default withRouter(ProjectsBillingAccountCreate);
