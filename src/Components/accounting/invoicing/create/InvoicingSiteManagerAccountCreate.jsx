...
import {
  createProposalOwnerAccount,
  deleteProposalOwnerAccount,
  updatePrimarySite,
  updateProposalOwnerAccount,
...
import { createInvoiceOwnerAccount, deleteInvoiceOwnerAccount, updateInvoiceOwnerAccount, updateInvoicePrimarySite } from "../../../../Controller/api/invoiceServices";
...
class InvoicingSiteManagerAccountCreate extends Component {
  ...
  handleOnSelect = (obj) => {
    ...
    createInvoiceOwnerAccount(values).then(() => {
      this.props.fetchInvoice(this.props.Invoice.id);
    });
  };

  handleDeleteOwnerAccount = () => {
    deleteInvoiceOwnerAccount(this.state.deletableAccountId).then(() => {
      this.props.fetchInvoice(this.props.Invoice.id);
    });
  };
  ...
  handleContactSelect = (id) => {
    ...
    const owner_contact_accounts = Invoice.invoice_owner_contact;
    ...
    updateInvoiceOwnerAccount(contactModalItem.id, values).then((response) => {
      this.props.fetchInvoice(this.props.Invoice.id);
    });
  };
  ...
  handleDeleteContact = () => {
    ...
    updateInvoiceOwnerAccount(item.id, values).then(() => {
      this.props.fetchInvoice(this.props.Invoice.id);
    });
  };
  ...
  handleSiteSelect = (id) => {
    ...
    updateInvoiceOwnerAccount(siteModalItem.id, values).then(() => {
  ...
  componentDidMount() {
    console.log(this.props.Invoice,"props from invoice sitemanager")
  }
  handleSubmit = () => {
    message.success("Site Manager Updated Successfully!");
    this.props.setInvoice(this.props.Invoice, 5);
  };
  ...
  handlePrimarySite = (id, siteId) => {
    updateInvoicePrimarySite(id, { primary_site_id: siteId })
      .then()
      .catch((err) => {
        handleError(err);
      });
  ...
  callbackOwnerAccount = (data) => {
    ...
    createProposalOwnerAccount(values).then(() => {
  ...
  callbackContact = (data) => {
    ...
    const owner_contact_accounts = proposal.owner_contact;
    const accountIndex = owner_contact_accounts.findIndex(
      (i) => i.id === contactModalItem.id
    );
    const account = owner_contact_accounts[accountIndex];
    ...
    updateProposalOwnerAccount(contactModalItem.id, values).then(
      (res) => {
        this.props.fetchProposal(this.props.proposal.id);
  ...
  callbackSite = (data) => {
    ...
    updateProposalOwnerAccount(siteModalItem.id, values).then(() => {
      this.props.fetchProposal(this.props.proposal.id);
    });
  }
  ...
  render() {
    ...
    const selectedAccounts = this.props.Invoice?.invoice_owner_contact || [];
    return (
      <React.Fragment>
        <div className="row common-form-card-row mx-0">
          <div className="col-12">
            <div className="row info-gray-div align-items-center">
              <h6 className="mb-0">
                Please add a Site Manager account and its contacts. You can only have one Site Manager for each proposal.
              </h6>
            </div>
          </div>
...
export default InvoicingSiteManagerAccountCreate;
