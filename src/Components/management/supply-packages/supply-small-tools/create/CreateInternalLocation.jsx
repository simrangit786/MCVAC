...
import { formatPhone, SUPPLY } from "../../../../../Controller/utils";
...
class CreateInternalLocation extends Component {
  ...
  handleRemove = () => {
    // let newArr = this.state.selectedContacts.drawer(i => i.id !== item.id)
  ...
  componentDidMount() {
    if (this.props.match.params.id) {
      ...
      if(this.props?.supply?.internal_location) {
        selectedContacts.push(this.props?.supply?.internal_location)
      }
      this.setState({selectedContacts})
    //   const params = {
    //     supply_id: this.props.match.params.id,
    //     type: SUPPLY,
    //   };
    ...
    //       //   internal_location: this.state.selectedContacts[0].name,
    //       // });
    //     })
    //     .catch((err) => {
    //       handleError(err);
  ...
  handleSelect = (e) => {
    this.formRef.current.setFieldsValue({internal_location: null})
  ...
  handleSubmit = (values) => {
    ...
    updateSupply(this.props.supply.id, data)
      .then((res) => {
        this.props.setSupply(res.data, 3);
        this.props.getSupply();
  ...
  showWarningModal = (warningVisible) => {
        ...
        <div className="row common-form-card-row">
          <div className="col-12">
            <div className="row info-gray-div align-items-center">
              <h6 className="mb-0">
                You can only choose one warehouse per supply / small tool.
              </h6>
            </div>
          </div>
                  ...
                  <div className="col-12">
                    ...
                    >
                      ...
                      <div className="col-12 col-sm-3">
                        ...
                        <p className="mb-0">{`${item?.street_address || ""} ${item?.name || ""} ${item?.city || ""} ${item?.state || ""} ${item?.zip || ""} ${item?.country || ""}`}</p>
                      </div>
                      <div className="col-12 col-sm-3">
