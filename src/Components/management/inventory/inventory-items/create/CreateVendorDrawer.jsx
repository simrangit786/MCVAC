...
class CreateVendorDrawer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      checked: false,
  ...
  populateVendorData = () => {
    ...
    if (this.props.editData) {
      this.formRef.current.setFieldsValue({
        ...
        ein: this.props.editData?.ein,
        account_1099: this.props.editData?.account_1099
      });
      this.mainRef.current.setFieldsValue({
        ...this.props.editData.main_address,
        phone_number: this.props.editData?.main_address?.phone,
        apartment_1: this.props.editData.billing_address?.apartment,
        street_address_1: this.props.editData.billing_address?.street_address,
        country_1: this.props.editData.billing_address?.country,
        city_1: this.props.editData.billing_address?.city,
        state_1: this.props.editData.billing_address?.state,
        zip_code_1: this.props.editData.billing_address?.zip_code,
        county_1: this.props.editData.billing_address?.county,


      })
      this.setState({ vendor: this.props.editData })
    }
    //  else {
  ...
  handleCreateVendor = (values) => {
    ...
    if (this.props.editData) {
      updateVendorAccount(type, this.props.editData.id)
        .then((res) => {
          this.setState(prevState => {
            return { vendor: res.data, activeKey: [...prevState.activeKey, '2'] }
          })
  ...
  handleSubmit = (values) => {
    ...
    if (this.props.editData?.main_address && this.props.editData?.billing_address) {
      updateMainAdd(main, this.props.editData?.main_address.id)
        .then((resp) => {
          updateBillingAdd(billing, this.props.editData?.billing_address.id).then((res) => {
            this.setState({
              main: resp.data,
              billing: res.data,
            });
    ...
    return (
      <React.Fragment>
        <Drawer
          centered
          destroyOnClose={true}
          title={editData ? "Edit Vendor" : "Create Vendor"}
...
export default CreateVendorDrawer;
