...
import {
  createOpportunityCustomerAccount,
  deleteOpportunityCustomerAccount,
  updateOpportunityCustomerAccount,
...
class CustomerAccount extends Component {
  ...
  showRemoveAccount = (visible, deletableAccountId = null) => {
    // console.log(deletableAccountId, "id");
    this.setState({
  ...
  handleOnSelect = (obj) => {
    ...
    createOpportunityCustomerAccount(values).then(() => {
  ...
  //   const customer_contact_accounts = opportunity.customer_contact_accounts
  //   const account = customer_contact_accounts.find(i => i.account.id === obj.value)
  //   this.handleDeleteContactAccount(account.id)
  // }

  handleDeleteContactAccount = (id) => {
    deleteOpportunityCustomerAccount(this.state.deletableAccountId).then(() => {
      this.props.fetchOpportunity();
    });
  };
  ...
  handleContactSelect = (id) => {
    ...
    this.selectRef.current.setFieldsValue({
      contact: null,
    });
    const { opportunity } = this.props;
    const customer_contact_accounts = opportunity.customer_contact_accounts;
    ...
    updateOpportunityCustomerAccount(contactModalItem.id, values).then(
      (response) => {
        this.props.fetchOpportunity();
      }
    ).catch(err => {
  ...
  handleDeleteContact = () => {
    ...
    updateOpportunityCustomerAccount(item.id, values).then((response) => {
      this.props.fetchOpportunity();
    }).catch(err => {
      handleError(err)
  ...
  getAllContacts = (params) => {
    ...
    this.setState({ loading: true });
    getContact({
      account: contactModalItem?.account?.id,
      ...params
    })
      .then((response) => {
  ...
  handleSubmit = () => {
    ...
    this.props.setOpportunity(this.props.opportunity, 4);
  };
  ...
  render() {
    ...
    const selectedCustomerAccounts =
      this.props.opportunity?.customer_contact_accounts || [];
    ...
    return (
      <React.Fragment>
        <div className="row common-form-card-row">
          <div className="col-12">
            <div className="row info-gray-div align-items-center">
              <h6 className="mb-0">
                Please add billing accounts and choose contacts.{" "}
              </h6>
            </div>
          </div>
          <div className="col-12 p-0">
            ...
            >
              <div className="row" style={{ padding: "0px 25px" }}>
                <div className="col-12 p-0">
                  ...
                  >
                    <Select
                      ...
                      onSearch={this.debounceEvent((e) => this.getAllAccounts({ search: e }), 1000)}
                ...
                {/*when-data-available*/}
                {selectedCustomerAccounts.map((item, index) => (
                            ...
                            <>
                              <div className="col-12 p-0">
                                ...
                                {/* <Radio.Group className="w-100"
                                                                    //  defaultValue={item.contact.find(n => n.default_customer_recipient === true)?.contact?.id}
                                                                     > */}
        ...
        <CreateContactDrawer
          ...
          selectedCustomerAccounts={selectedCustomerAccounts}
